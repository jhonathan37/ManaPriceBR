...
class _SearchPageState extends State<SearchPage> {
  ...
  @override void initState(){super.initState();_controller=TextEditingController(text:widget.initialName??'');_loadDefaults();}
  Future<void> _loadDefaults() async {final settings=await AppSettingsStore.load();if(!mounted)return;setState((){_condition=settings.condition;_foil=settings.foil;_language=settings.language;_loadingDefaults=false;});if((widget.initialName??'').trim().isNotEmpty){WidgetsBinding.instance.addPostFrameCallback((_){if(mounted)_loadPrintings(widget.initialName!.trim());});}}
  ...
  @override Widget build(BuildContext context){final theme=Theme.of(context);return Scaffold(appBar:AppBar(title:const Text('Buscar carta')),body:_loadingDefaults?const Center(child:CircularProgressIndicator()):ListView(padding:const EdgeInsets.fromLTRB(20,12,20,32),children:[Text('Qual carta você procura?',style:theme.textTheme.headlineSmall?.copyWith(fontWeight:FontWeight.w800)),const SizedBox(height:6),Text('Digite em português ou inglês. A busca principal é imediata; escolher a edição é opcional e melhora a precisão.',style:theme.textTheme.bodyLarge?.copyWith(color:theme.colorScheme.onSurfaceVariant)),const SizedBox(height:20),TextField(controller:_controller,autofocus:widget.initialName?.isEmpty??true,textInputAction:TextInputAction.search,onChanged:_onChanged,onSubmitted:(_)=>_search(),decoration:InputDecoration(labelText:'Nome da carta',hintText:'Ex.: Testemunha Eterna ou Eternal Witness',prefixIcon:const Icon(Icons.search_rounded),suffixIcon:_controller.text.isEmpty?null:IconButton(tooltip:'Limpar',onPressed:(){_controller.clear();setState((){_suggestions=const[];_printings=const[];_selectedPrinting=null;_resolvedImageUrl=null;_catalogNotice=null;});},icon:const Icon(Icons.close_rounded)))),if(_loadingSuggestions)...[const SizedBox(height:8),const LinearProgressIndicator()],if(_catalogNotice!=null)...[const SizedBox(height:10),Container(padding:const EdgeInsets.all(12),decoration:BoxDecoration(color:theme.colorScheme.surfaceContainerHigh,borderRadius:BorderRadius.circular(14)),child:Row(crossAxisAlignment:CrossAxisAlignment.start,children:[Icon(Icons.info_outline_rounded,color:theme.colorScheme.primary),const SizedBox(width:10),Expanded(child:Text(_catalogNotice!,style:theme.textTheme.bodyMedium))]))],if(_suggestions.isNotEmpty)...[const SizedBox(height:8),Card(clipBehavior:Clip.antiAlias,child:Column(children:_suggestions.map((name)=>ListTile(leading:Icon(Icons.style_rounded,color:theme.colorScheme.primary),title:Text(name,style:const TextStyle(fontWeight:FontWeight.w600)),trailing:const Icon(Icons.north_west_rounded,size:18),onTap:()=>_selectSuggestion(name))).toList()))],const SizedBox(height:24),_StepTitle(number:'1',title:'Edição (opcional)',subtitle:'Escolha se quiser uma cotação mais precisa. Isso nunca bloqueia a busca.'),const SizedBox(height:12),if(_loadingPrintings)const LinearProgressIndicator()else if(_printings.isNotEmpty)DropdownButtonFormField<CardPrinting>(initialValue:_selectedPrinting,isExpanded:true,decoration:const InputDecoration(labelText:'Edição da carta',prefixIcon:Icon(Icons.layers_rounded)),items:_printings.map((p)=>DropdownMenuItem(value:p,child:Text(p.label,overflow:TextOverflow.ellipsis))).toList(),onChanged:(value)=>setState(()=>_selectedPrinting=value))else OutlinedButton.icon(onPressed:_loadEditionsOnly,icon:const Icon(Icons.layers_outlined),label:const Text('Carregar edições (opcional)')),const SizedBox(height:22),_StepTitle(number:'2',title:'Ajuste a carta',subtitle:'Os valores abaixo usam suas preferências salvas.'),const SizedBox(height:10),Card(child:ExpansionTile(title:const Text('Idioma, condição e acabamento',style:TextStyle(fontWeight:FontWeight.w700)),subtitle:Text('$_language • $_condition • ${_foil?'Foil':'Não foil'}'),childrenPadding:const EdgeInsets.fromLTRB(16,0,16,16),children:[DropdownButtonFormField<String>(initialValue:_language,decoration:const InputDecoration(labelText:'Idioma'),items:const['Português','Inglês','Espanhol'].map((v)=>DropdownMenuItem(value:v,child:Text(v))).toList(),onChanged:(v){if(v!=null)setState(()=>_language=v);}),const SizedBox(height:12),DropdownButtonFormField<String>(initialValue:_condition,decoration:const InputDecoration(labelText:'Condição'),items:const['NM','SP','MP','HP','DMG'].map((v)=>DropdownMenuItem(value:v,child:Text(v))).toList(),onChanged:(v){if(v!=null)setState(()=>_condition=v);}),SwitchListTile(contentPadding:EdgeInsets.zero,title:const Text('Carta foil'),subtitle:const Text('Ative somente se a impressão for foil.'),value:_foil,onChanged:(v)=>setState(()=>_foil=v))])),const SizedBox(height:24),SizedBox(height:54,child:FilledButton.icon(onPressed:_search,icon:const Icon(Icons.price_check_rounded),label:Text(_selectedPrinting!=null?'Ver preço desta edição':'Buscar agora'))),const SizedBox(height:6),Text(_selectedPrinting==null?'A busca não espera o carregamento das edições.':'Edição selecionada: ${_selectedPrinting!.setName}',textAlign:TextAlign.center,style:theme.textTheme.bodySmall?.copyWith(color:theme.colorScheme.onSurfaceVariant)),const SizedBox(height:10),TextButton.icon(onPressed:()=>context.push('/effect-search'),icon:const Icon(Icons.auto_awesome_rounded),label:const Text('Não sabe o nome? Descreva o que a carta faz'))]));}
}

class _StepTitle extends StatelessWidget{const _StepTitle({required this.number,required this.title,required this.subtitle});final String number,title,subtitle;@override Widget build(BuildContext context){final theme=Theme.of(context);return Row(crossAxisAlignment:CrossAxisAlignment.start,children:[Container(width:32,height:32,alignment:Alignment.center,decoration:BoxDecoration(color:theme.colorScheme.primaryContainer,shape:BoxShape.circle),child:Text(number,style:TextStyle(fontWeight:FontWeight.w800,color:theme.colorScheme.primary))),const SizedBox(width:12),Expanded(child:Column(crossAxisAlignment:CrossAxisAlignment.start,children:[Text(title,style:theme.textTheme.titleMedium?.copyWith(fontWeight:FontWeight.w800)),const SizedBox(height:2),Text(subtitle,style:theme.textTheme.bodySmall?.copyWith(color:theme.colorScheme.onSurfaceVariant))]))]);}}
