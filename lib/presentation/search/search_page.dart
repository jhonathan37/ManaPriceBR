...
class _SearchPageState extends State<SearchPage> {
  ...
  Future<void> _search() async {final name=_controller.text.trim();if(name.isEmpty){_notice('Digite o nome da carta que você quer consultar.');return;}await _ensurePrintings();if(!mounted)return;if(_printings.isNotEmpty&&_selectedPrinting==null){_notice('Só falta escolher a edição. Assim o preço não vem de uma impressão diferente.');return;}FocusScope.of(context).unfocus();setState(()=>_suggestions=const[]);final p=_selectedPrinting;context.push('/result',extra:{'name':name,'setCode':p?.setCode,'setName':p?.setName,'collectorNumber':p?.collectorNumber,'imageUrl':p?.imageUrl??_resolvedImageUrl,'condition':_condition,'foil':_foil,'language':_language});}
  void _notice(String text)=>ScaffoldMessenger.of(context).showSnackBar(SnackBar(content:Text(text)));

  @override Widget build(BuildContext context){final theme=Theme.of(context);return Scaffold(appBar:AppBar(title:const Text('Buscar carta')),body:_loadingDefaults?const Center(child:CircularProgressIndicator()):ListView(padding:const EdgeInsets.fromLTRB(20,12,20,32),children:[
